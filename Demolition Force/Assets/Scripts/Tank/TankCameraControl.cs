...
public class TankCameraControl : MonoBehaviour {
	...
	void CameraRevert(){
		...
		{
			currentX = Mathf.Lerp (currentX,player.rotation.eulerAngles.y, 8f * Time.deltaTime);
			if(Mathf.Abs(Input.GetAxis("Mouse Y")) > 0.3 || Mathf.Abs(Input.GetAxis("Mouse X")) > 0.3)
				resetCamera = false;

			if (Mathf.Abs(currentX - player.rotation.eulerAngles.y) < 1f)
			{
				currentX = player.rotation.eulerAngles.y;
				resetCamera = false;
			}
		}
	...
	public void UpdateCameraMovement(float cameraX, float cameraY){
		...
		//Get settings preference for inversion

		//xInverted = PlayerPrefsManager.GetInvertedX();
		//yInverted = PlayerPrefsManager.GetInvertedY();
